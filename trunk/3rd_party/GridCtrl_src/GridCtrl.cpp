...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_bGridDrawDone=FALSE;  //to check grid load status
        ...
        if( GetColumnWidth( nCol) > 0 )
        {
            ...
            for (nRow = nStartRow; nRow <= nEndRow; nRow++)
            {
                if( GetRowHeight( nRow) > 0 )
                {
					...
					//seven-2013-1-30 modified to expand all the grids by default
					...
					m_arColWidths[nCol] = maxColWidth;
					m_arRowHeights[nRow] = maxRowHeight;
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	static GV_DISPINFO gvdicell;
	gvdicell.item.row=cell.row;
	gvdicell.item.col=cell.col;

	if(m_pfnClickback)
	{
		m_pfnClickback(&gvdicell,m_lClickParam);
		return;	
	}
