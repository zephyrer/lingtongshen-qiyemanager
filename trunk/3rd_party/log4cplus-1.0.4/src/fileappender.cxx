...
{
	std::ios::openmode m = mode;
#ifdef UNICODE
	m = mode | std::ios_base::binary;
#endif

    //out.open(LOG4CPLUS_TSTRING_TO_STRING(filename).c_str(), m);
	out.open(filename.c_str(), m);
...
#ifdef UNICODE
		out.put(0xFEFF); //the BOM flag needed for UTF16-LE
		LPCWSTR SAMPLE = L"\n";
		out << SAMPLE;
		out.flush();
#endif
